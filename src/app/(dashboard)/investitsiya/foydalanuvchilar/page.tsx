...
const inpClass = "w-full px-3 py-2.5 rounded-xl text-sm outline-none transition-all inp-modal";
const inpStyle = { background: "#f1f5f9", border: `1.5px solid #e2e8f0`, color: "#0f172a" };
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.role} onChange={e => { f("role", e.target.value); if (e.target.value !== "TASHKILOT") f("organizationType", ""); }}
                    className={`${inpClass} cursor-pointer appearance-none`} style={inpStyle}>
                    {ROLES.map(r => <option key={r} value={r}>{ROLE_CFG[r].label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-bold tracking-widest mb-1.5" style={{ color: T2 }}>BO'LIM (DEPARTMENT)</label>
                  <select value={form.department ?? ""} onChange={e => f("department", e.target.value)}
                    className={`${inpClass} cursor-pointer appearance-none`}
                    style={{ ...inpStyle, color: form.department ? "#0f172a" : "#94a3b8" }}>
                    <option value="" style={{ color: "#9ca3af" }}>— Tanlang —</option>
              ...
              {form.role === "TASHKILOT" && (
                <div>
                  <label className="block text-[11px] font-bold tracking-widest mb-1.5" style={{ color: T2 }}>
                    TASHKILOT TURI *
                    <span className="ml-1.5 font-normal normal-case" style={{ color: "#ef4444" }}>(majburiy)</span>
                  </label>
                  <select value={form.organizationType ?? ""} onChange={e => f("organizationType", e.target.value)}
                    className={`${inpClass} cursor-pointer appearance-none`}
                    style={{ ...inpStyle, color: form.organizationType ? "#0f172a" : "#94a3b8", border: !form.organizationType ? "1.5px solid #fca5a5" : "1.5px solid #e2e8f0" }}>
                    <option value="" style={{ color: "#9ca3af" }}>— Tashkilot turini tanlang —</option>
                    {PROBLEM_TYPES.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
                  </select>
                </div>
              )}
              {err && (
