...
export default function MomoYuborishPage() {
  ...
  useEffect(() => {
    getMyProjects()
      .then(mine => {
        if (mine.length > 0) { setProjects(mine); setProjLoading(false); return; }
        return getProjects().then(all => { setProjects(all); setProjLoading(false); });
      })
      .catch(() => {
        getProjects()
          .then(setProjects)
  ...
  const onFilePick = (e: React.ChangeEvent<HTMLInputElement>) => {
    const picked = Array.from(e.target.files ?? []).slice(0, MAX_IMAGES - images.length);
  ...
  const step1Done = !!title.trim() && !!type && !!projectId;
  const step2Done = !!description.trim() && !!department;
  const step3Done = !!finalPos;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!step1Done) return setError("Sarlavha, muammo turi va loyihani kiriting");
    ...
    try {
      ...
      if (images.length > 0) {
        await Promise.allSettled(
          images.map(file => {
            ...
            fd.append("problemId", String(result.id));
      ...
      setError(msg ?? "Yuborishda xatolik yuz berdi. Qayta urinib ko'ring.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-0 mb-6 p-4 rounded-2xl"
        ...
        {[
          { n: 1, l: "Asosiy",    done: step1Done },
          { n: 2, l: "Tavsif",    done: step2Done },
          { n: 3, l: "Lokatsiya", done: step3Done },
          { n: 4, l: "Rasmlar",   done: images.length > 0 },
        ].map((s, i) => (
          <div key={s.n} className="flex items-center flex-1">
            ...
            {i < 3 && <div className="flex-1 h-px mx-3 hidden sm:block"
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-4">
        ...
        <div className="p-5 rounded-2xl space-y-4" style={boxStyle}>
          ...
          <div>
            ...
            {projLoading ? (
              <div className="w-full px-4 py-2.5 text-sm rounded-xl animate-pulse"
                style={{ ...inputStyle, color: "rgba(100,130,200,0.45)" }}>
                Loyihalar yuklanmoqda...
              </div>
            ) : projects.length === 0 ? (
              <div className="w-full px-4 py-2.5 text-xs rounded-xl"
                style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171", borderRadius: 12 }}>
                ⚠️ Sizda hali loyiha mavjud emas. Admin bilan bog&apos;laning.
              </div>
            ) : (
              <select value={projectId} onChange={e => setProjectId(e.target.value)}
                ...
                onFocus={e => { e.target.style.borderColor = "rgba(59,130,246,0.4)"; e.target.style.boxShadow = "0 0 0 3px rgba(59,130,246,0.08)"; }}
                onBlur={e  => { e.target.style.borderColor = "rgba(255,255,255,0.08)"; e.target.style.boxShadow = "none"; }}>
                <option value="" style={{ background: "#0d1f3c" }}>— Loyihani tanlang —</option>
                {projects.map(p => (
                  <option key={p.id} value={p.id} style={{ background: "#0d1f3c" }}>
        ...
        <div className="p-5 rounded-2xl" style={boxStyle}>
          <h3 className="font-semibold text-sm mb-1 flex items-center gap-2" style={{ color: "rgba(200,220,255,0.88)" }}>
            ...
            Lokatsiya <span className="text-xs ml-1" style={{ color: "#f87171" }}>*</span>
          </h3>
        ...
        {/* ── 4: Rasmlar ── */}
        <div className="p-5 rounded-2xl space-y-3" style={boxStyle}>
          <h3 className="font-semibold text-sm flex items-center gap-2" style={{ color: "rgba(200,220,255,0.88)" }}>
            <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
              style={{
                background: images.length > 0 ? "rgba(16,185,129,0.2)" : "rgba(59,130,246,0.18)",
                color:      images.length > 0 ? "#34d399" : "#60a5fa",
                border: `1px solid ${images.length > 0 ? "rgba(16,185,129,0.4)" : "rgba(59,130,246,0.3)"}`,
              }}>
              {images.length > 0 ? "✓" : "4"}
            </span>
            Rasmlar
            <span className="text-xs ml-1" style={{ color: "rgba(100,130,200,0.5)" }}>(ixtiyoriy, max {MAX_IMAGES} ta)</span>
          </h3>
          ...
          {images.length < MAX_IMAGES && (
            <>
              ...
              <button type="button" onClick={() => fileRef.current?.click()}
                className="w-full flex items-center justify-center gap-3 py-5 rounded-xl transition-all"
                style={{ background: "rgba(59,130,246,0.05)", border: "1.5px dashed rgba(59,130,246,0.25)", color: "rgba(96,165,250,0.7)" }}
                ...
                <div className="text-left">
                  <p className="text-sm font-semibold" style={{ color: "rgba(150,180,250,0.85)" }}>Rasm yuklash</p>
                  <p className="text-xs" style={{ color: "rgba(96,130,200,0.5)" }}>
                    {images.length > 0 ? `${images.length} ta tanlandi · yana ${MAX_IMAGES - images.length} ta qo'shish mumkin` : "JPG, PNG, WEBP · max 5 ta"}
                  </p>
                </div>
              </button>
            </>
        ...
        <button type="submit" disabled={loading}
          ...
          style={{ background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.3)", color: "#60a5fa" }}
          onMouseEnter={e => { if (!loading) { (e.currentTarget as HTMLButtonElement).style.background = "rgba(59,130,246,0.28)"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 24px rgba(59,130,246,0.2)"; } }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(59,130,246,0.15)"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "none"; }}>
          {loading
            ? <span className="flex items-center justify-center gap-2">
                ...
                {images.length > 0 ? "Muammo va rasmlar yuborilmoqda..." : "Yuborilmoqda..."}
              </span>
            : "🚀 Muammo yuborish"
          }
        </button>
