...
export default function QurilishShartnomalarnPage() {
    ...
    const load = async () => {
      ...
      try {
        const [all, projs] = await Promise.all([
          getContracts(),
          getProjects({ type:"QURILISH" }),
        ]);
        setProjects(projs);
        // Filter contracts belonging to QURILISH projects
        const qurilishIds = new Set(projs.map(p=>p.id));
        setContracts(all.filter(c => qurilishIds.has(c.projectId)));
      } catch { setContracts([]); }
    ...
    <div className="-m-6 flex flex-col" style={{ minHeight:"calc(100vh - 60px)", background:"#080d1a" }}>
      ...
      <div className="px-6 pt-6 pb-4 flex items-start justify-between flex-shrink-0">
        <div>
          ...
          <h1 className="text-xl font-bold" style={{ color:"rgba(220,235,255,0.97)" }}>Qurilish Shartnomalari</h1>
          <p className="text-xs mt-0.5" style={{ color:"rgba(100,130,200,0.55)" }}>Qurilish loyihalari shartnomalari</p>
        </div>
              ...
              {loading ? (
                ...
                return (
                  <tr key={c.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    ...
                    <td className="px-4 py-3.5 text-xs font-mono font-semibold" style={{color:"#a78bfa"}}>{c.contractNumber}</td>
          ...
          <div className="w-full max-w-[480px] rounded-2xl overflow-hidden"
            ...
            <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto scrollbar-hide">
              ...
              <div>
                ...
                <textarea value={form.description??""} onChange={e=>f("description",e.target.value)}
                  placeholder="Shartnoma haqida qisqacha ma'lumot" rows={2}
            ...
            <div className="px-6 py-4 flex justify-end gap-3" style={{borderTop:"1px solid rgba(255,255,255,0.07)"}}>
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                style={{background:saving?"rgba(124,58,237,0.5)":"#7c3aed", opacity:saving?0.7:1}}>
      ...
      {delId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-[360px] rounded-2xl p-6"
            ...
            <p className="text-sm text-center mb-6" style={{color:"rgba(120,150,200,0.6)"}}>
              Bu shartnoma butunlay o&apos;chib ketadi.
            </p>
